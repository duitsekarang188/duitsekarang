...
interface FormDataState {
  nama: string; ic: string; phone: string;
  kerja: string; pendapatan: string; syarikatNama: string; syarikatAlamat: string; syarikatPhone: string; tempohKerja: string;
  bankNama: string; bankAkaun: string;
  rujukan1Nama: string; rujukan1Hubungan: string; rujukan1Phone: string;
  rujukan2Nama: string; rujukan2Hubungan: string; rujukan2Phone: string;
}

interface FileNamesState {
  mykadDepan: string; mykadBelakang: string; bilUtiliti: string;
  gajiBulan1: string; gajiBulan2: string; gajiBulan3: string;
  bankBulan1: string; bankBulan2: string; bankBulan3: string; kwsp: string;
  [key: string]: string;
}
...
export default function MohonPage() {
  ...
  // State Nama Fail Berasingan
  const [fileNames, setFileNames] = useState<FileNamesState>({
    mykadDepan: '', mykadBelakang: '', bilUtiliti: '',
    gajiBulan1: '', gajiBulan2: '', gajiBulan3: '',
    bankBulan1: '', bankBulan2: '', bankBulan3: '',
    kwsp: ''
  });
  
  // State Kamera Live Selfie
  const [cameraActive, setCameraActive] = useState<boolean>(false);
  const [capturedSelfieName, setCapturedSelfieName] = useState<string>('');
  ...
  const handleSingleFileChange = (e: React.ChangeEvent<HTMLInputElement>, key: string) => {
            ...
            <div className="text-center space-y-8 py-4 animate-fade-in">
              ...
              <div className="space-y-3">
                ...
                <p className="text-sm text-slate-400 max-w-md mx-auto leading-relaxed">
                  Data e-KYC, dokumen kewangan, and maklumat perhubungan anda telah berjaya disimpan dengan selamat. Saringan kelayakan kredit anda akan diproses segera oleh pihak pentadbiran kami.
                </p>
              </div>
            </div>
          )}
